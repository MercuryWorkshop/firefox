...
class DecodedSurfaceProvider final : public ISurfaceProvider,
 ...
 private:
  ...
  /// Host-GPU image passthrough: cached WebRender image key + the external image
  /// id (when this surface is backed by a host-uploaded GL texture). The external
  /// image is unregistered on destruction. See imgFrame::HostGpuExternalImage.
  Maybe<wr::ImageKey> mGpuKey;
  uint64_t mGpuExternalId = 0;
#endif
